...
things like how to write better tests that find more bugs.  But don't
...
{\bf COMMENTARY:}  This dialogue is a tribute to, and reflection on, Imre Lakatos'
...
meaning that (some of) the students arrive at is based on a Popperian~\cite{Popper,popperconjectures} falsification methodology proposed in
...
wouldn't have thought possible, we have that now, and not just for
...
{\bf COMMENTARY:} While linear search is a poor specification for binary search,
differential testing is indeed, as the students speculate, a powerful tool, widely used in compiler, file
...
discovered many times in the history of testing.  Differential testing
is one of the most useful ``tricks'' for working around the
\emph{oracle problem}~\cite{oracleMcMinn,staats2011programs}.  The
oracle problem, though the students do not mention it by name, is
a fundamental aspect of their concerns: how can a specification be
``embodied'' as checks that will fail during a test.  Translating a
mental model of what a program should do into computable conditions on
executions is
a key part of the work of refuting programs.
...
The idea of having a computer check proofs dates, in a certain sense,
to Leibniz (again), but at least to the frequently reprinted 1962 proposal of McCarthy~\cite{McCarthy1962TowardsAM,mccarthy1993towards}.
\end{minipage}}
\vspace{0.1in}
...
the same issue really comes up for the simple version, if the problem
...
\vspace{0.1in}
\framebox{\begin{minipage}{0.4 \textwidth}
{\bf COMMENTARY:}   A somewhat different (but still related) approach, less explicitly
seen in the dialogue, is that presented in, e.g., Turner's
\emph{Computational Artifacts}~\cite{turner2018computational}, where
programs are seen as \emph{machines} with a human-intended
\emph{purpose}.  It is interesting to consider that while \emph{metaphors}
from this viewpoint are common (e.g., ``factory methods'') in the
field, the literature of program correctness is much more attuned to
programs as mathematical than as industrial artifacts (or, more
generally, as teleological artifacts, like the wheel or the axe, of
\emph{homo faber}).  This may be due to the Leibnizian dream of proofs
and proof-checking; while engineers use scientific principles in
design, no one really thinks that the model is the thing, or that a
bridge can be ``proven'' rather than tested.  We wish it to be
otherwise, with computer programs, so prefer not to think of them in
this context as 
unfortunately over-complex, but strangely easy to copy, ``machines.''
\end{minipage}}
\vspace{0.1in}
...
\vspace{0.1in}
\framebox{\begin{minipage}{0.4 \textwidth}
{\bf COMMENTARY:}   The general issue of trust focusing on specifications, rather than
...
can produce programs, which are inherently \emph{hard} to check, they can also produce
tests~\cite{gpttestgen,siddiq2023empirical}, which as the dialogue
suggests, may simplify the problem of trust.  Or, as Delta has implied, it may not...
\end{minipage}}
\vspace{0.1in}
...
mathematical way to approach the problem of the general suitability of
